...
export async function findBestRoute(amount: bigint, routes: Route[], swapType: SwapType) {
  ...
  try {
    response = await multicall.getFunction("tryAggregate").staticCall(
      false,
  ...
  response.map((data, i) => {
    ...
    if (success) {
      try {
        [amountQuoted] = quoter.decodeFunctionResult(method, returnData);
      } catch (e) {
  ...
  console.log("Best route found:", bestRoute.amountQuoted, "for route:", bestRoute.route);
...
function encodeExactQuote(route: Route, amount: bigint, swapType: SwapType) {
  ...
  switch (swapType) {
    case SwapType.EXACT_INPUT:
      return {
        callData: quoter.encodeFunctionData(QuoteMethod.EXACT_INPUT, [encodedRoute, amount]),
        method: QuoteMethod.EXACT_INPUT,
      };

    case SwapType.EXACT_OUTPUT:
      return {
        callData: quoter.encodeFunctionData(QuoteMethod.EXACT_OUTPUT, [encodedRoute, amount]),
